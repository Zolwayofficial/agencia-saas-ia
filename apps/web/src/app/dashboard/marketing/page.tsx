                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
                    ...
                    <div className="flex gap-2 p-1.5 bg-white/[0.03] rounded-xl border border-white/[0.05] shrink-0">
                        {['All', 'Active', 'Paused', 'Complete'].map(filter => (
                            <button key={filter} className={`px-4 py-1.5 rounded-lg text-[10px] font-bold tracking-wider uppercase transition-all whitespace-nowrap ${filter === 'All' ? 'bg-brand-primary text-black shadow-[0_5px_15px_rgba(var(--brand-primary-rgb),0.2)]' : 'text-muted hover:text-header'
                                }`}>
                                {filter}
                            </button>
                        ))}
