                ...
                <div className="flex gap-1 p-1 bg-white/[0.03] rounded-xl border border-white/[0.05] shrink-0">
                    {['All', 'Connected', 'Setup'].map(f => (
                        <button
                            key={f}
                            onClick={() => setFilter(f)}
                            className={`px-5 py-2 rounded-lg text-[10px] font-bold tracking-widest uppercase transition-all whitespace-nowrap ${filter === f ? 'bg-brand-primary text-black shadow-[0_5px_15px_rgba(var(--brand-primary-rgb),0.2)]' : 'text-muted hover:text-header'
                                }`}
                        >
